...
// __global__
// void applyGeometryKernel_2D_wrap(const double* trInv, T minxpp, T maxxpp, T minypp,
...
//             break;
//         case 1: {
//             T res = interpolatedElementBSpline2D_Degree1(xp, yp, coefsXDim,
//                     coefsYDim, coefs);
//             size_t index = i * xdim + j;
//             data[index] = res;
//         }
...
void applyGeometryKernel_2D_wrap(const double* trInv, T minxpp, T maxxpp, T minypp,
    ...
    if (wrap) {
        if ( xp < minxpp || xp >= maxxpp ) {
            xp = realWRAP(xp, minxp - 0.5, maxxp + 0.5); // FIXME specialize for float/double
        }

        if ( yp < minypp || yp >= maxypp ) {
            yp = realWRAP(yp, minyp - 0.5, maxyp + 0.5);
        }
